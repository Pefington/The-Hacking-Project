...
def number_of_handles(array, desired_length)
  increment = 0
  array.each { |handle| increment += 1 if handle.length == desired_length } # @1
  puts increment
end
...
def upper_cases(array)
  increment = 0
  # array.each { |handle| increment += 1 if isupcase(handle[1]) } # .matches(/[A-Z]/)
  array.each { |handle| increment += 1 if handle[1].match?(/[A-Z]/) }
  puts increment
end

def sort_array_alphabetically(array)
  result_array = array.sort_by(&:downcase)
  puts result_array
end
