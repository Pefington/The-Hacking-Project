...
gossips.size.times do |index|
  ...
  iteration.times do
    ...
    puts "A new gossip tag has been created for gossip n° #{index}"
  end
end

50.times do
  PrivateMessage.create(content: Faker::Lorem.paragraph_by_chars, sender: users.sample, recipient: users.sample)
...
i = 0
while Like.all.size < 20
  i += 1
  iteration = rand(1..2)
  user = users.sample
  if iteration == 1
    gossip = gossips.sample
    Like.create(user: user, gossip: gossip) if Like.where(user: user, gossip: gossip).size.zero?
    # puts "\nA new like has been created in a gossip"
  else
    comment = comments.sample
    Like.create(user: user, comment: comment) if Like.where(user: user, comment: comment).size.zero?
